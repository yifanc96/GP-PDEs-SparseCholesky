...
$N_\partial+N+N_d$ functionals and the nugget $R=\sigma^2 I$ on the data
rows (zero elsewhere),
...
$H=2J_v\T\Th_u\inv J_v+2J_w\T\Th_w\inv J_w+D$ \eqref{eq:H}: the Jacobian
...
\paragraph{Reproduction.} Run \texttt{examples/darcy\_inverse\_hybrid.py}
with no flags: the defaults are exactly the configuration of this note ---
coarse warm-up, data-bordered $u$-step, split lengthscales
$\ell_u=1.0$/$\ell_w=0.2$, $\rho=3$, $6$ warm-up steps, $3$ alternating
steps. The scaling tables are produced by sweeping \texttt{-{}-N-domain}
and \texttt{-{}-N-data}.
